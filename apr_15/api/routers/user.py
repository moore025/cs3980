...
async def get_all_users(user: Annotated[TokenData, Depends(get_user)]) -> list[UserDto]:
    ...
    result = []
    for u in users:
        result.append(UserDto(id=str(u.id), username=u.username, email=u.email))
    return result


@user_router.post("/{id}")
async def update_user_role(
    id: PydanticObjectId, newRole: str, user: Annotated[TokenData, Depends(get_user)]
    ...
    if affected_user == "BasicUser":
        affected_user.role = newRole  # What?
